...
import { FlaskConical, Plus, Clock, Thermometer, Gauge, User, CheckSquare } from 'lucide-react';
...
export default function Pulping() {
  const { pulpingBatches, materials, addPulpingBatch, updatePulpingBatch } = useStore();
  const { openBatch } = useBatchDrawer();
  const [selectedId, setSelectedId] = useState<string>(pulpingBatches[0]?.id ?? '');
  const [showForm, setShowForm] = useState(false);
  const [form, setForm] = useState(emptyForm);
  const [formBarkRatio, setFormBarkRatio] = useState(60);
  const [formMatIds, setFormMatIds] = useState<string[]>([]);
  ...
  const selectedMats = selected
    ? materials.filter((m) => selected.materialIds.includes(m.id))
    : [];
  ...
  const handleSubmit = () => {
    if (!form.batchNo || !form.operator) return;
    addPulpingBatch({
      ...form,
      id: `PB${Date.now()}`,
      materialIds: formMatIds,
      barkRatio: formBarkRatio,
      strawRatio: 100 - formBarkRatio,
    });
    setForm(emptyForm);
    setFormBarkRatio(60);
    setFormMatIds([]);
  ...
  const handleSliderChange = (val: number) => {
    if (!selected) return;
    updatePulpingBatch(selectedId, { barkRatio: val, strawRatio: 100 - val });
  };

  const toggleFormMat = (id: string) => {
    setFormMatIds((prev) =>
    ...
    <div className="animate-fade-in-up space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 space-y-4">
          <div className="flex items-center justify-between">
            <h3 className="font-serif text-lg font-semibold text-xuan-ink">制浆批次</h3>
          ...
          {showForm && (
            <div className="xuan-card p-6 animate-fade-in-up">
              ...
              <div className="space-y-4">
                <div>
                  <label className="block text-sm text-xuan-inkLight mb-2">关联原料批次（可多选）</label>
                  <div className="grid grid-cols-2 md:grid-cols-3 gap-2 max-h-40 overflow-y-auto">
                    {materials.map((m) => (
                      <label
                        key={m.id}
                        className={`flex items-center gap-2 p-2 rounded-lg border cursor-pointer transition-colors text-sm ${
                          formMatIds.includes(m.id)
                            ? 'border-xuan-ochre bg-xuan-ochre/10 text-xuan-ochre'
                            : 'border-xuan-paperDark/50 hover:border-xuan-ochre/40 text-xuan-inkLight'
                        }`}
                      >
                        <input type="checkbox" className="accent-xuan-ochre" checked={formMatIds.includes(m.id)} onChange={() => toggleFormMat(m.id)} />
                        <span className="truncate">{m.batchNo}</span>
                      </label>
                    ))}
                  </div>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  <div>
                    <label className="block text-sm text-xuan-inkLight mb-1">青檀皮比例: {formBarkRatio}%</label>
                    <input type="range" min={0} max={100} value={formBarkRatio} onChange={(e) => setFormBarkRatio(+e.target.value)} className="w-full accent-xuan-ochre" />
                    <div className="w-full h-3 rounded-full overflow-hidden bg-xuan-paperDark/30 flex">
                      <div className="h-full bg-xuan-ochre/70" style={{ width: `${formBarkRatio}%` }} />
                      <div className="h-full bg-xuan-gold/70" style={{ width: `${100 - formBarkRatio}%` }} />
                    </div>
                  </div>
                  <div>
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <button
                    ...
                    className="font-serif font-semibold text-xuan-ink hover:text-xuan-ochre hover:underline underline-offset-2 transition-colors"
                  >
                    {b.batchNo}
                  </button>
        ...
        <div className="space-y-4">
            ...
            {selected && (
              <>
                ...
                {selectedMats.length > 0 && (
                  <div className="mb-4">
                    <p className="text-xs text-xuan-inkLight mb-2 font-medium">关联原料 ({selectedMats.length})</p>
                    <div className="space-y-1.5">
                      {selectedMats.map((m) => (
                        <div key={m.id} className="flex items-center justify-between text-xs">
                          <span className="text-xuan-ink">{m.batchNo}</span>
                          <span className="text-xuan-inkLight">{m.type} · {m.qualityGrade}</span>
                        </div>
                      ))}
                    </div>
                  </div>
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    <label className="text-sm text-xuan-inkLight">调整配比</label>
                    <span className="text-sm font-medium text-xuan-ochre">{selected.barkRatio} : {selected.strawRatio}</span>
                  </div>
                  <input
                    type="range"
                    min={0}
                    max={100}
                    value={selected.barkRatio}
                  ...
                  <div className="w-full h-4 rounded-full overflow-hidden bg-xuan-paperDark/30 flex">
                    <div className="h-full bg-xuan-ochre/70 transition-all" style={{ width: `${selected.barkRatio}%` }} />
                    <div className="h-full bg-xuan-gold/70 transition-all" style={{ width: `${selected.strawRatio}%` }} />
                  </div>
                  <div className="flex justify-between text-xs text-xuan-inkLight">
                    <span>青檀皮 {selected.barkRatio}%</span>
                    <span>稻草 {selected.strawRatio}%</span>
                  </div>
                </div>
              </>
            )}
          ...
          <div className="xuan-card p-5">
            ...
            <table className="xuan-table w-full text-sm">
              ...
              <tbody>
                {pulpingBatches.map((b) => (
                  <tr key={b.id} className="cursor-pointer hover:bg-xuan-ochre/5" onClick={() => {
                    setSelectedId(b.id);
                  }}>
                    <td
                      className="font-medium text-xuan-ink cursor-pointer hover:text-xuan-ochre"
                      onClick={(e) => {
                        e.stopPropagation();
                        openBatch(b.batchNo);
                      }}
      ...
      <div className="xuan-card p-4">
        <div className="flex items-center gap-3">
          <CheckSquare className="w-4 h-4 text-xuan-moss" />
          <p className="text-sm text-xuan-inkLight">
            提示：调整配浆滑块会实时更新当前选中批次的配比，数据自动保存，刷新页面不丢失。点击批次号可查看完整溯源详情。
          </p>
        </div>
      </div>
    </div>
